...
export const getBookDetails = async (book_id) => {
    try{
        const response = await api.get(`/v1/library/books/${book_id}`);
        return response.data;
    } catch (error) {
        console.error("❌ Error fetching book details:", error);
        throw (
            error.response?.data || {
                message: "Failed to fetch book details. Please try again later.",
            }
        );
    }
}
...
export const updateBook = async (book_id, bookData) => {
    try {
        const response = await api.put(`/v1/library/books/${book_id}`, bookData);
        return response.data;
    } catch (error) {
        console.error("❌ Error updating book:", error);
        throw (
            error.response?.data || {
                message: "Failed to update book. Please check your input and try again.",
            }
        );
    }
}
...
export const deleteBook = async (book_id) => {
    try {
        const response = await api.delete(`/v1/library/books/${book_id}`);
        return response.data;
    } catch (error) {
